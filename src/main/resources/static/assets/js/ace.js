...
jQuery(function($) {
	...
	function enableDemoAjax() {		
		...
		var demo_ajax_options = {
			 ...
			 'content_url': function(hash) {
				...
				//please refer to documentation for more info
                 ...
                 if( !hash.match(/^page\//) && !hash.match(/^server\//) ) return false;
		...
		$(window).on('error.ace_ajax', function(err) {
...
(function($ , undefined) {
	...
	function AceAjax(contentArea, settings) {
		...
		this.getUrl = function(url, hash, manual_trigger, method, data, updateView, extraParams) {
			...
			$.ajax(ajax_params).error(function(res) {
				$contentArea.trigger('ajaxloaderror', {url: url, hash: hash, method: method, data: data, res:res});
